...
**July 17, 2026**
- Safetensors are now the recommended checkpoint format. The sibling `.pth` files were rebuilt as tensor-only MMEngine-compatible checkpoints and pass PickleScan with zero dangerous globals. See the [model zoo](MODEL_ZOO.md#checkpoint-security-and-formats) for loading instructions and verification details.
...
The [ColorMAE Hugging Face repository](https://huggingface.co/carlosh93/colormae) is the canonical checkpoint archive. OSF links are intentionally retained as mirrors. See the [complete model zoo](MODEL_ZOO.md) for the recommended Green models and all Blue, Purple, and Red ablations. Safetensors are the recommended downloads; sibling `.pth` files remain available as tensor-only MMEngine-compatible checkpoints. Every task directory includes a canonical config, with raw resolved configs and scalar-log shards under `runs/<timestamp>/`. File sizes, metrics, tensor fingerprints, local provenance, and SHA-256 checksums are recorded in the [checkpoint manifest](https://huggingface.co/carlosh93/colormae/blob/main/ViT_Base/MODEL_ZOO_MANIFEST.jsonl), [security conversion manifest](https://huggingface.co/carlosh93/colormae/blob/main/SECURITY_CONVERSION_MANIFEST.jsonl), and [artifact manifest](https://huggingface.co/carlosh93/colormae/blob/main/ViT_Base/MODEL_ZOO_ARTIFACTS_MANIFEST.jsonl).
...
The detailed tables below keep `.pth` links because the documented OpenMMLab commands load them directly. Those files are weights-only compatibility checkpoints and pass PickleScan; they are not full training-resume snapshots.
