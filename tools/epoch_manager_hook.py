...
class EpochManagerHook(Hook):
    """
    This hook is useful when you are using SLURM and Multi-Node training and you have time constraints.
    It can run custom command at the end of specific number of epochs and submit the next job.
    command (str): The command to run at the end of specific number of epochs.
    max_slurm_epochs (int): The number of pretrain epochs to run before running the command.
    """
    def __init__(self, command=None, max_slurm_epochs=None):
        self.command = command
        self.max_slurm_epochs = max_slurm_epochs
        self.counter = 0
        self.start_time = None
        self.backups = 0
        
    def before_train_epoch(self, runner):
        if self.command is not None and self.counter+1 > self.max_slurm_epochs:
            ...
            if 'mae' in runner.experiment_name:
                # self.backup_specific_ckpt(runner, [100, 200]) # Optional: backup specific ckpts
                self.check_loaded_color(runner)
                # self.print_example_masks(runner, [0, 100, 200]) # Optional: print example masks at the beginning of training
            self.start_time = time.time()
            if self.max_slurm_epochs is not None:
                runner.logger.info(f'SLURM training epoch {self.counter + 1}/{self.max_slurm_epochs}')
            self.counter += 1
    ...
    def check_loaded_color(self, runner_obj):
        ...
        if run_model.backbone.masking_generator.loaded_color != current_data_path[2]:
            runner_obj.logger.info(f"====> Changing Color Patterns. Loading {current_data_path[2]} patterns from {current_data_path[1]} <====")
            run_model.backbone.masking_generator.change_color_pattern(current_data_path[1])
            self.print_example_masks(runner_obj, [current_epoch])


    def print_example_masks(self, runner_obj, epochs=None):
        if runner_obj.rank == 0:
            ...
            if runner_obj.epoch in epochs:
                runner_obj.logger.info(f"Printing some example {run_model.backbone.masking_generator.loaded_color} noise-based masks")
                res = run_model.backbone.masking_generator.color_noise_mask(run_model.backbone.masking_generator.extract_windows(5))
                ...
                plt.savefig(f"{runner_obj.work_dir}/example_masks_{runner_obj.epoch}.png")
                plt.close()