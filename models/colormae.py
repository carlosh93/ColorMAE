...
from .utils.masking_generator import ColorMasking
import math

@MODELS.register_module()
class ColorMAEViT(VisionTransformer):
    """Vision Transformer for MAE pre-training with color masking.

    A PyTorch implementation of: `ColorMAE: Exploring data-independent masking 
    strategies in Masked AutoEncoders <https://arxiv.org/abs/2407.13036>`_.
    This module implements the data-independent masking strategies in ColorMAE.
    ...
    Args:
        ...
        mask_ratio (bool): The ratio of total number of patches to be masked.
            Defaults to 0.75.
        mask_type (str | dict): The type of masking strategy. It can be "random" or a dict specifying the color noise pattern.
    ...
    def __init__(self,
                 ...
                 mask_type: Union[str, dict] = 'random',
        ...
        if isinstance(self.mask_type, dict):
            ...
            print(f"=========> USING RANDOM MASKING: {self.mask_ratio} Masking Ratio <=========")
    ...
    def color_masking(
        ...
        """Generate the mask for MAE Pre-training based on color noise pattern
        Args:
        ...
        if not math.sqrt(L).is_integer():
            raise ValueError('L is not a square number. Means high and width of patches are not the same')

        mask, ids_restore, ids_keep = self.masking_generator.color_noise_mask(self.masking_generator.extract_windows(N))
    ...
    def random_masking(
        ...
        Returns:
            ...
            - ``ids_restore`` (torch.Tensor): ids to restore original image.
        """
        # Check if mask type is random
        if self.mask_type != 'random':
            NotImplementedError("Should not be using random as mask type is not random")
        N, L, D = x.shape  # batch, length, dim
    ...
    def forward(
        ...
        if mask is None or False:
            ...
            if isinstance(self.mask_type, str) and self.mask_type == 'random':
